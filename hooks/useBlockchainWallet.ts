...
export function useBlockchainWallet() {
  ...
  useEffect(() => {
    const connectPromise = async () => {
      ...
      if (!isFetchingSession && !userSession && (blockchainType === Blockchain.Root || loginLibaryflag[LoginLibrary.Pass])) { //We don't need the blockchain type as use effect dependency because to be connected, blockchain type must be defined
        dispatch(disconnect());
