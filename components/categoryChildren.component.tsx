function optionList(props: any) {
  return props.list.map((x: any) => {
    return (
      <div className="overflow-hidden w-16 h-16 flex justify-center flex-col items-center bg-gray-300 mx-2" key={x.id} onClick={() => {props.handleClick(x.id, x.path, x.name)}}>
        ...
        <p className="text-[0.5rem]">{x.name}</p>
      </div>
    )
  });
}

function CategoryChildrenComponent(props: any) {
  return (
