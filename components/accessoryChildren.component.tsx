...
interface ExampleProps {
  list: AccessoryInterface[];
  handleClick: (id: string, path: string, name: string) => void;
}

function optionList(props: ExampleProps) {
  return props.list.map((x: AccessoryInterface) => {
    return (
      <div className="w-16 h-16 bg-gray-200 border-2 border-gray-100 rounded-[6px] drop-shadow-md mb-4" key={x.id}
           onClick={() => {
             props.handleClick(x.id, x.path, x.name)
           }}>
        <div className="flex flex-col items-center justify-center h-full">
          <Image src={'/resources/icons/accessories/' + x.type + '.svg'} width={30} height={30} alt={x.type}/>
