  ...
  return (
    list != undefined ?
      list.map((opt) => {
        return (
          <SwiperSlide
            className={'flex justify-center items-center transition-transform' + (activeOption?.val === opt.name ? ' -translate-y-2' : '')}
            key={opt.id}>
            <p className='nm-flat-slate-100 hidden absolute'></p>
            <div className="rounded-md w-[75px] h-[75px] flex items-center justify-center relative overflow-hidden"
              ...
              <div className='w-[75px] h-[75px] rounded-md overflow-hidden flex justify-center items-center'>
                {/* <Image src={'/resources/icons/features/' + opt.type + '.svg'} width={50} height={50} alt={opt.name}/> */}
                <Image src={'/resources/images/image.png'} width={75} height={75} alt={opt.name}/>
              </div>
              <p
                className='text-[10px] px-2 absolute bottom-0 right-0 bg-slate-100 max-w-full rounded-tl-md'>{opt.name}</p>
              <div
                className={'w-2 h-2 absolute top-1 right-1 rounded-full bg-green-400 border border-green-600 opacity-0' + (activeOption?.val === opt.name ? '  opacity-100' : '')}></div>
            </div>
          </SwiperSlide>
        )
      })
