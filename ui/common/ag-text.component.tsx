...
//* AGText is a text component with various styles and functionalities.
export default class AGText extends Component<AGTextProps, AGTextState> {
  /**
   * @returns The CSS class for the text alignment based on the specified side.
   */
  getSide() {
  ...
  /**
   * @returns The JSX element for the text mark based on the specified mark type.
   */
  renderMark() {
  ...
  /**
   * @returns The JSX element for the text based on the specified text type.
   */
  renderType() {
    ...
    switch (type) {
        ...
        return (
          <div>
            <button
              ...
              onClick={() => void this.copyToClipboard(children as string)}
            >
              {this.state?.copied ? '✔' : '📄'}
            </button>
            <p
              className={'my-2 mx-1 p-2 bg-slate-400 text-sm text-indigo-900 rounded' + (this.props.justText ? ' break-all' : ' whitespace-pre-wrap')}
              style={{ fontFamily: '"Lucida Console", Monaco' }}
            >
              {children}
            </p>
          </div>
      ...
      case "link":
        return (
          <>
            {" "}
            <a target="_blank" rel='noreferrer' href={href} className="underline">{children}</a>
          </>
        );
      case "end":
