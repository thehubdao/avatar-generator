  ...
  return (
    <div className={`my-2 flex ${vD.side ?? ''}`}>
      <button type={form ? 'submit' : undefined}
        className={`mx-2 w-auto min-h-[32px] my-auto rounded py-1 border-1 ${vD.hover} ${vD.textColor} ${vD.color} ${vD.borderColor} ${nm ? 'shadow-flat-soft rounded-lg' : ''} ${nm && selected ? '!shadow-inset-soft rounded-lg' : ''} ${fit ? 'w-fit' : 'min-w-[100px]'} ${full ? '!w-full mx-0' : ''} ${circle ? 'rounded-full' : ''} group/button transition-all duration-300`}
