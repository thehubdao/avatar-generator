...
function AssetCard({ id, name, thumb, location, updateDefaultAsset }: AssetCardProps) {
  ...
  const checkAssetFile = () => {
    ...
    setHasFile(!!fileLength && fileLength >= 1);
    setHasThumb(!!thumbLength && thumbLength >= 1);
  };
  ...
  async function deleteDoc(docId: string) {
    ...
    dispatch(fetchData({ campaign: campaignName, location: location }));
  }
  ...
  return (
    <>
      <div className="relative w-56 p-4 shadow-flat-soft overflow-hidden bg-bg rounded-lg hover:shadow-flat-hard transition-all duration-300 cursor-pointer group">
        <div className="pb-2">
          {name && (
            <p className="font-poppins font-medium text-xl text-gray-normal uppercase truncate max-w-full">{name}</p>
          )}
        </div>
          ...
          {willDelete ? (
            ...
            <>
              <div className="w-full">
                ...
                <input
                  type="text"
                  className="shadow-inset-soft hover:shadow-inset-medium px-2 py-1 my-2 min-h-[48px] w-full rounded-lg text-center bg-bg"
                  min="5"
                  max="20"
