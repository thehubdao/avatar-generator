...
export async function FindAndReadjustFeatureIndexes(campaign: string) {
  ...
  for (const feature of featureList) {
    if (feature.index == undefined) {
      void LogError(Module.CollectionUtil, `Missing index on ${feature.id} feature type!`);
      continue;
    }

    featureOptionListData.set(feature.index, featureData.filter(f => f.type === feature.id));
  }
