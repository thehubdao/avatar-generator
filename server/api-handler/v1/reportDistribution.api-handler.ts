...
interface PostPayload { campaign: string, features: FeatureInterface[] }
...
export async function PostApiHandler(
    req: NextApiRequest,
    res: NextApiResponse
): Promise<void> {
    const { campaign, features } = req.body as PostPayload

    // Add +1 to the counter
    const updates = features.map((feat: FeatureInterface): Promise<unknown> =>
        AddTierDistribution(campaign, feat)
    )
